...
const form = useForm({
    _method: props.reminder.id ? 'PUT' : 'POST',
...
const submit = () => {
    if (props.reminder.id) {
        form.post(route('reminders.update', props.reminder.id), {
            preserveScroll: true,
            onSuccess: () => {
...
<template>
    <form @submit.prevent="submit" class="space-y-6">
        ...
        <div class="flex items-center">
            <input type="checkbox" id="completed" v-model="form.completed_at"
                :true-value="new Date().toISOString().split('T')[0]" :false-value="null"
        ...
        <div>
            ...
            <select id="board" v-model="form.board_id"
                class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-blue-500 focus:ring-blue-500">
